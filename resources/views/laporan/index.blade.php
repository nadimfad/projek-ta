...
<div class="space-y-6">
    <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div class="flex flex-col sm:flex-row sm:flex-wrap gap-3">
            <form method="GET" action="{{ route('laporan.index') }}" class="grid grid-cols-1 sm:grid-cols-[minmax(0,260px)_minmax(0,220px)_auto] gap-3 w-full sm:w-auto">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari laporan..." class="w-full rounded-lg border-gray-200 text-sm shadow-sm">

                <select name="id_kegiatan" class="w-full border-gray-200 rounded-lg shadow-sm text-sm">
            ...
            <a href="{{ route('laporan.create') }}" class="bg-gray-900 text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-gray-800 transition text-center">
                Tambah Laporan
            </a>
        </div>
    </div>
